...
if(!defined("FOPO_CORE")) header("Location:http://106.10.51.32/");
	...
	function F_find($db_object, $mem_no, $mem_id) {
		$sql = $db_object->fetch($db_object->query("SELECT mem_list.mem_no, mem_list.mem_nick, mem_list.mem_picfile, (select count(mem_no) from phozone_article WHERE mem_list.mem_no = phozone_article.mem_no) AS art_cnt FROM mem_list WHERE mem_list.mem_id = '". $mem_id . "'"), 1);
		...
		$row_array['mem_picfile'] = $sql['mem_picfile'];
	...
	function F_list($db_object,$mem_no) {
		$sql = $db_object->query("select mem_list.mem_no,mem_list.mem_nick,(select count(mem_no) from phozone_article group by mem_no HAVING mem_no = fri_no)as art_cnt, mem_list.mem_picfile from friend_list,mem_list where friend_list.mem_no =".$mem_no." and friend_list.fri_no = mem_list.mem_no");
		...
		while ($row =  $db_object->fetch($sql, 1)) {
			...
			$row_array['mem_picfile'] = $row['mem_picfile'];
