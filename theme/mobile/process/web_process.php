...
*/
	...
	function web_login($db_object, $auth_id) {
		...
		if ( $tmpSessionData['sess_no'] > 0 ) { // 인덱스가 0이상 일경우 (칼럼이 있으면..)
			...
			$sess_token = uniqid();
			$rand_num = sprintf('%04d', rand(1000, 9999)); // 랜덤 4자리수 생성
	...
	function logout($db_object, $token) {
		$db_object->query("DELETE FROM session_list WHERE sess_token='$token'");
